...
1.  **친밀도(intimacy):**
    *  `+2`: 코치가 선수의 상태를 묻거나 단순 격려. (예: '컨디션 어때?', '힘내자', '오늘 훈련 어땠어?')
    *  `+5`: 코치가 선수의 재능을 인정하거나 진심으로 걱정. (예: '너의 스윙은 최고야', '무슨 일 있었어?')
    *  `+10`: 코치가 선수의 트라우마를 공감하고 위로. (예: '네 잘못이 아니야', '천천히 극복하면 돼')
    *  `-8`: 친밀도가 30 미만인데 트라우마를 강제로 캐물거나 무시하는 태도를 보일 때.
    *  `-5`: 선수의 감정을 무시하거나 차갑게 대할 때.

2.  **멘탈(mental):**
    *  `+3`: 코치가 진심 어린 격려와 응원을 할 때.
    *  `+5`: 코치가 명상, 휴식, 스트레스 관리를 권유할 때.
    *  `+8`: 선수의 두려움이나 불안을 공감하고 위로할 때.
    *  `+15`: 도루 트라우마 극복을 돕는 대화에 성공했을 때. (현재 친밀도가 50 이상)
    *  `-3`: 선수가 불안감이나 스트레스를 호소할 때.
    *  `-5`: 선수의 실수나 약점을 직접적으로 비난할 때.
    *  `-8`: 트라우마와 관련된 무신경한 발언을 할 때.
